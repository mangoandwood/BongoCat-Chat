# BongoCat 双人聊天版（非官方修改版）

> 本项目基于 [ayangweb/BongoCat](https://github.com/ayangweb/BongoCat) 修改，保留原项目 MIT 许可证与作者署名。联网聊天、双端键鼠同步和用户自建服务器功能不是原项目官方功能，本仓库与 Steam 版本没有官方关联。

本公开版不提供由软件作者承担额度的中央服务器。每对用户只需其中一人点击一次 Cloudflare 部署按钮，服务器便运行在该用户自己的免费账户中；另一人只需导入邀请信息。

- [普通使用说明](./使用说明.md)
- [用户自建服务器：保姆式教程](./用户自建服务器-保姆式教程.md)
- [公开发布注意事项](./公开发布说明.md)
- Cloudflare 一键部署按钮：点击后会在部署者自己的 Cloudflare 免费账户中创建转发节点。
